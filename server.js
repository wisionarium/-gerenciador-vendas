...
const { db, creditForParticipants } = require('./db');
...
function activeSellerIds() {
  return db.prepare("SELECT id FROM users WHERE role='seller' AND active=1").all().map(r => r.id);
}
...
// ---------- SELLERS / USERS ----------
app.get('/api/sellers', requireAuth, (req, res) => {
  // vendedora vê lista ativa p/ montar participantes; admin vê todas
  if (req.user.role === 'admin') {
    const rows = db.prepare("SELECT * FROM users WHERE role='seller' ORDER BY name").all();
    return res.json({ sellers: rows.map(toPublicUser) });
  }
  const rows = db.prepare("SELECT * FROM users WHERE role='seller' AND active=1 ORDER BY name").all();
  return res.json({ sellers: rows.map(toPublicUser) });
});

app.get('/api/users', requireAuth, requireAdmin, (req, res) => {
  const rows = db.prepare('SELECT * FROM users ORDER BY role DESC, name').all();
...
app.post('/api/users', requireAuth, requireAdmin, (req, res) => {
  ...
  try {
    const r = db.prepare('INSERT INTO users (name, email, password_hash, role, active) VALUES (?,?,?,?,1)')
      .run(name.trim(), email.trim().toLowerCase(), bcrypt.hashSync(String(password), 10), role);
...
app.put('/api/users/:id', requireAuth, requireAdmin, (req, res) => {
  ...
  try {
    db.prepare('UPDATE users SET name=?, email=?, role=? WHERE id=?')
      .run(name.trim(), email.trim().toLowerCase(), role || target.role, target.id);
    if (password) {
      if (String(password).length < 4) return res.status(400).json({ error: 'Senha deve ter ao menos 4 caracteres.' });
...
app.post('/api/calls', requireAuth, (req, res) => {
  ...
  // vendedora só registra para si; admin pode registrar para qualquer vendedora ativa
  let sellerId = req.user.id;
  if (req.user.role === 'admin') {
    if (seller_id) {
      const s = db.prepare("SELECT * FROM users WHERE id=? AND role='seller' AND active=1").get(seller_id);
  ...
  const r = db.prepare("INSERT INTO call_records (seller_id, date, quantity) VALUES (?,?,?)").run(sellerId, date, quantity);
  const row = db.prepare('SELECT * FROM call_records WHERE id=?').get(r.lastInsertRowid);
  res.status(201).json({ call: row });
});

app.delete('/api/calls/:id', requireAuth, (req, res) => {
  const row = db.prepare('SELECT * FROM call_records WHERE id=?').get(req.params.id);
  ...
  db.prepare('DELETE FROM call_records WHERE id=?').run(row.id);
  res.json({ ok: true });
});

// ---------- SALES ----------
function saleWithParticipants(sale) {
...
app.get('/api/sales', requireAuth, (req, res) => {
  ...
  res.json({ sales: rows.map(saleWithParticipants) });
});
...
app.post('/api/sales', requireAuth, (req, res) => {
  ...
  // valida participantes ativas
  const placeholders = pids.map(() => '?').join(',');
  const sellers = db.prepare(`SELECT * FROM users WHERE id IN (${placeholders}) AND role='seller' AND active=1`).all(...pids);
  if (sellers.length !== pids.length) return res.status(400).json({ error: 'Participante inválida ou desativada.' });

  // vendedora só pode criar venda onde ela participa
  ...
  let credit;
  try { credit = creditForParticipants(pids.length); }
  catch (e) { return res.status(400).json({ error: e.message }); }

  const r = db.prepare(
    'INSERT INTO sales (customer_name, product, color, channel, created_by, sale_date) VALUES (?,?,?,?,?,?)'
  ).run(customer_name.trim(), product.trim(), color.trim(), channel, req.user.id, date);
  const saleId = r.lastInsertRowid;
  const insP = db.prepare('INSERT INTO sale_participants (sale_id, seller_id, credit) VALUES (?,?,?)');
  for (const sid of pids) insP.run(saleId, sid, credit);
  const sale = db.prepare('SELECT * FROM sales WHERE id=?').get(saleId);
  res.status(201).json({ sale: saleWithParticipants(sale) });
});
...
// ---------- STATS ----------
function summarize(from, to, sellerId) {
  const p = [];
  let callWhere = '1=1';
  ...
  let calls = 0;
  if (sellerId) {
    const r = db.prepare(`SELECT COALESCE(SUM(quantity),0) AS t FROM call_records WHERE seller_id=? AND ${callWhere}`).get(sellerId, ...p);
    calls = r.t;
  } else {
    const r = db.prepare(`SELECT COALESCE(SUM(quantity),0) AS t FROM call_records WHERE ${callWhere}`).get(...p);
    calls = r.t;
  }

  // vendas: soma de créditos
  const sp = [];
  let saleWhere = '1=1';
  if (from) { saleWhere += ' AND s.sale_date >= ?'; sp.push(from); }
  if (to) { saleWhere += ' AND s.sale_date <= ?'; sp.push(to); }
  let creditSel = 'SELECT COALESCE(SUM(sp.credit),0) AS t FROM sale_participants sp JOIN sales s ON s.id=sp.sale_id WHERE ' + saleWhere;
  let creditParams = [...sp];
  if (sellerId) { creditSel += ' AND sp.seller_id=?'; creditParams.push(sellerId); }
  const credit = db.prepare(creditSel).get(...creditParams).t;

  const waSel = creditSel.replace('sp.credit', "CASE WHEN s.channel='WhatsApp' THEN sp.credit ELSE 0 END");
  const crmSel = creditSel.replace('sp.credit', "CASE WHEN s.channel='CRM' THEN sp.credit ELSE 0 END");
  const wa = db.prepare(waSel).get(...creditParams).t;
  const crm = db.prepare(crmSel).get(...creditParams).t;

  // nº de registros únicos de venda (para relatório)
  let countSel = 'SELECT COUNT(DISTINCT s.id) AS t FROM sales s ' + (sellerId ? 'JOIN sale_participants sp ON sp.sale_id=s.id AND sp.seller_id=? WHERE ' + saleWhere : 'WHERE ' + saleWhere);
  const countParams = sellerId ? [sellerId, ...sp] : [...sp];
  const records = db.prepare(countSel).get(...countParams).t;
...
app.get('/api/stats/summary', requireAuth, (req, res) => {
  ...
  res.json(summarize(from || null, to || null, sid));
});

app.get('/api/stats/ranking', requireAuth, requireAdmin, (req, res) => {
  const { from, to } = req.query;
  const sellers = db.prepare("SELECT * FROM users WHERE role='seller' AND active=1 ORDER BY name").all();
  const rows = sellers.map(s => {
    const st = summarize(from || null, to || null, s.id);
  ...
  }).sort((a, b) => b.sales - a.sales || b.calls - a.calls);
  res.json({ ranking: rows });
});

app.get('/api/stats/seller/:id', requireAuth, (req, res) => {
  ...
  const current = summarize(from || null, to || null, targetId);
  const compare = (cfrom || cto) ? summarize(cfrom || null, cto || null, targetId) : null;
  const pct = (cur, prev) => {
  ...
  const dailyCalls = db.prepare(`SELECT date, SUM(quantity) AS calls FROM call_records WHERE ${cw} GROUP BY date ORDER BY date`).all(...p2);
  const seller = db.prepare('SELECT * FROM users WHERE id=?').get(targetId);
...
app.get('/api/report/daily', requireAuth, requireAdmin, (req, res) => {
  const date = req.query.date || todayISO();
  if (!isValidDate(date)) return res.status(400).json({ error: 'Data inválida.' });
  const summary = summarize(date, date, null);
  const sellers = db.prepare("SELECT * FROM users WHERE role='seller' AND active=1 ORDER BY name").all();
  const ranking = sellers.map(s => {
    const st = summarize(date, date, s.id);
    return { seller_id: s.id, name: s.name, calls: st.calls, sales: st.salesCredit };
  }).filter(r => r.sales > 0 || r.calls > 0).sort((a, b) => b.sales - a.sales);
